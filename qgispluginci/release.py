...
def create_archive(
  ...
  if add_submodules:
    ...
    for submodule in repo.submodules:
      ...
      with tarfile.open(top_tar_file, mode="a") as top_tar:
        with tarfile.open(sub_tar_file, mode="r:") as sub_tar:
          for sub_tar_member in sub_tar.getmembers():
            if not sub_tar_member.isfile():
              continue
            top_tar.add(sub_tar_member.name)
  ...
  if not Path(f"{parameters.plugin_path}/LICENSE").is_file():
    ...
    if parent_license.is_file():
      ...
      with tarfile.open(top_tar_file, mode="a") as top_tar:
        top_tar.add(f"{parameters.plugin_path}/LICENSE")
  ...
  with zipfile.ZipFile(
    ...
    with tarfile.open(top_tar_file, mode="r:") as top_tar:
      for sub_tar_member in top_tar.getmembers():
        ...
        # code : https://github.com/python/cpython/blob/b885b8f4be9c74ef1ce7923dbf055c31e7f47735/Lib/zipfile
        # .py#L545
...
def release(
  ...
  upload_plugin_repo_github
      If true, a custom repo will be created as a release asset on Github and could later be used in QGIS as
  ...
  allow_uncommitted_changes
      ...
      If True and some changes are detected, a hard reset on a stash create will be used to revert changes
      made by qgis-plugin-ci.
  disable_submodule_update
      If omitted, a git submodule is updated. If specified, git submodules will not be updated/initialized
      before packaging.
  asset_paths
      Assets to included in the ZIP file
  """
